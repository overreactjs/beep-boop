...
export const usePatrolState = (movement: UsePlatformMovementResult): StateFunction<SecurityBotState> => {
  ...
  return useCallback((fsm, delta) => {
    ...
    const isAbovePlayer = game.nearestPlayer(fsm.entity).block.current[1] >= by;
    const isAbovePlatform = game.isPlatformBelow(bx, by);
    if (isAbovePlayer && isAbovePlatform && chance(CHANCE_JUMP * delta)) {
