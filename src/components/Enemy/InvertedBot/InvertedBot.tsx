...
export const InvertedBot: React.FC<EnemyProps<InvertedBotState>> = ({ enemy, collider }) => {
  ...
  const movement = usePlatformMovement(collider, pos, velocity, {
    ...
    maxFallSpeed,
    speed,
    activeTags: ['bottom', 'left', 'right'],
  });

  // Setup the finite state machine, to handle the behaviour of each state.
  const fsm = useEnemyStateMachine<InvertedBotState>(enemy, {
    idle: useIdleState(),
  ...
  return (
    <Node pos={pos}>
      ...
      <Node offset={[-5, -8]}>
        <CollisionBox size={[10, 14]} id={collider} tags={tags} active={active} />
      </Node>
      <Node pos={enemy.pos} offset={[-8, 0]} rounded>
        <Dizzy fsm={fsm} />
      </Node>
    </Node>
  );
